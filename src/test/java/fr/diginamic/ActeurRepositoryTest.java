...
public class ActeurRepositoryTest {
    ...
    public void testExtraireActeursParAnneeNaissance() {
        ...
        TypedQuery<Acteur> query = em.createQuery(
                "SELECT a FROM Acteur a " +
                        "WHERE EXTRACT(YEAR FROM a.anniversaire) = :annee",
                Acteur.class
        );
	...
	public void testExtraireActeursParRole() {
		...
		TypedQuery<Acteur> query = em.createQuery("SELECT DISTINCT a FROM Acteur a JOIN a.roles r WHERE r.nom = 'Harley Quinn'", Acteur.class);
		List<Acteur> acteurs = query.getResultList();
	...
	public void testExtraireActeursParFilmParuAnnee() {
		TypedQuery<Acteur> query = em.createQuery("SELECT DISTINCT a FROM Acteur a JOIN a.roles r JOIN r.film f WHERE f.annee = 2015", Acteur.class);
		List<Acteur> acteurs = query.getResultList();
	...
	public void testExtraireActeursParPays() {
        ...
        query.setParameter("nomPays", "France");
	...
	public void testExtraireParRealisateurEntreAnnee() {
        TypedQuery<Acteur> query = em.createQuery(
                "SELECT DISTINCT a " +
                        ...
                        "WHERE re.identite = :nomRealisateur " +
                        "AND f.annee BETWEEN :anneeDebut AND :anneeFin",
                Acteur.class
        );

        query.setParameter("nomRealisateur", "Ridley Scott");
